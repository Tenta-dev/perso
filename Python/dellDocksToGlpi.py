...
################
### GLPI API ###
glpiApiUrl = "http://changeME/apirest.php"
appToken = "changeME"
userToken = "changeME"
...
try:
    ...
    if isinstance(e, IndexError):
        print(f"La dock station portant le numero de serie ({dockSerial}) n'éxiste pas: {e}")
        ### GET NEW DOCK NAME ###
        try:
            ...
            for docks in requestsResult:
                ...
                if docks["name"] == requestsGet.json()[-1]["name"]:
                    ...
                    while not isDeleted(newDockName):
                        newDockName = incrementNom(newDockName)
                    break
            print("newDockName: ", newDockName)
        except Exception as e:
        ...
        createBody = {
            "input": {
                ...
                "locations_id": 1,
                "peripheraltypes_id": 2,
                "peripheralmodels_id": int(dockModelId),
                "manufacturers_id": 41,
                "states_id": 1
            }
        }
